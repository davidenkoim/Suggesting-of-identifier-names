...
public class IdModelRunner {
    ...
    public List<Pair<File, List<List<List<Integer>>>>> predict(File file) {
        this.modelStats = new long[] { 0, -System.currentTimeMillis()  };
        this.mrr = 0.0;
        return this.lexerRunner.lexDirectory(file)
                .map(p -> {
    ...
    public List<List<List<Integer>>> predictFile(File f) {
        if (!this.lexerRunner.willLexFile(f)) return null;
        this.model.notify(f);
        return predictTokens(this.lexerRunner.lexFile(f));
    }

    public List<List<List<Integer>>> predictContent(String content) {
        return predictTokens(this.lexerRunner.lexText(content));
    }

    public List<List<List<Integer>>> predictTokens(Stream<Stream<String>> lexed) {
        this.getVocabulary().setCheckpoint();
        List<List<List<Integer>>> linePreds;
        if (this.lexerRunner.isPerLine()) {
            linePreds = lexed
                    .map(this.getVocabulary()::toIndices)
                    .map(l -> l.collect(Collectors.toList()))
            ...
            List<Integer> lineLengths = new ArrayList<>();
            List<List<Integer>> preds = predictSequence(lexed
                    .map(this.getVocabulary()::toIndices)
                    .map(l -> l.collect(Collectors.toList()))
                    .peek(l -> lineLengths.add(l.size()))
    ...
    protected List<List<Integer>> predictSequence(List<Integer> tokens) {
        if (this.selfTesting) this.model.forget(tokens);
    ...
    private void logPredictionProgress(List<List<Integer>> preds) {
        long prevCount = this.modelStats[0];
        ...
        if (this.modelStats[0] / this.MODEL_PRINT_INTERVAL > prevCount / this.MODEL_PRINT_INTERVAL
            ...
            System.out.printf("Predicting: %dK tokens processed in %ds\n",
                    Math.round(this.modelStats[0]/1e3),
                    (System.currentTimeMillis() + this.modelStats[1])/1000);
        }
    }
    ...
    private List<List<List<Integer>>> predsToLines(List<List<Integer>> modeled, List<java.lang.Integer> lineLengths) {
        List<List<List<Integer>>> perLine = new ArrayList<>();
        int ix = 0;
        for (int i = 0; i < lineLengths.size(); i++) {
